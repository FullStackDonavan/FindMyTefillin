<template>
  <div class="mx-auto bg-white dark:bg-gray-900 p-6 rounded-lg shadow-md">
    ...
    <div v-if="registered === true" class="mb-6">
      <label class="block text-gray-700 dark:text-gray-300 mb-2">Enter your registered ID Tag:</label>
      <input v-model="idTag" type="text" maxlength="7" class="form-input mb-3" placeholder="e.g. 1234567" />
      
      <button
        @click="markRegisteredLost"
        :disabled="loading"
        class="w-full bg-yellow-600 hover:bg-yellow-700 text-white font-semibold py-2 rounded-lg"
      >
        🛠 Mark as Lost
      </button>
    </div>
...
const markRegisteredLost = async () => {
  if (!idTag.value) return alert("Please enter your ID tag.")
  loading.value = true

  try {
    const res = await fetch('/api/registered-tefillin/mark-lost', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${authToken.value}`,
      },
      body: JSON.stringify({ idTag: idTag.value }),
    })

    const data = await res.json()
    if (!data.success) throw new Error(data.message)

    alert('✅ Your tefillin have been marked as lost.')
    router.push('/thank-you-lost')
  } catch (err) {
